...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/10/28 09:56:58 by tkong             #+#    #+#             */
/*   Updated: 2022/10/28 10:06:43 by tkong            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft.h"

static void		unit_merge_sort(t_u8 **buf, t_i32 seq, size_t len);
...
void	ft_sort_u8(t_u8 *arr, t_i32 begin, t_i32 end)
{
	...
	ft_memcpy(arr + begin, buf[seq & 1], sizeof(t_u8) * len);
	free(buf[0]);
	free(buf[1]);
}
